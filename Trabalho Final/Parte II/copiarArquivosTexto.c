...
struct Palavra *CopiarArquivoTexto(struct Palavra *lista, char nomeArquivo[], int posicaoArquivo);
...
struct Palavra *InserirOrdemAlfabetica(struct Palavra *lista, char palavra[], int linha, int posicaoArquivo);
        ...
        while(caractereAuxiliar != EOF) {
            ...
            if(strlen(palavra) > 1) {
                if(BuscarPalavra(lista, palavra) == NULL) {
                    lista = InserirOrdemAlfabetica(lista, palavra, linha, posicaoArquivo);
                }
                else {
...
int PosicaoArquivo(struct Arquivo *arquivos, char nomeArquivo[], int qdtArquivos) {
    ...
    for(i = 0; i < qdtArquivos; i++) {
        if(nomeArquivo == arquivos->nomeArquivo) {
            posicao = i;
            break;
        }
    }
